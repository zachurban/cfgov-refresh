...
class PlaceholderJSMixin(object):
    """Mixin that creates placeholder JS files required by Django templates.

    The frontend build generates JavaScript files which are included in
    Django templates via {% include %} blocks. If these files don't exist,
    Django unit tests that try to render those templates will fail with a
    TemplateNotFound exception.

    This mixin creates empty placeholder files to allow running of backend
    unit tests without first needing to run the frontend build. Files are
    ...
    def setup_test_environment(self, **kwargs):
        super(PlaceholderJSMixin, self).setup_test_environment(**kwargs)

        for p in self.PLACEHOLDER_FILES:
...
class TestDataTestRunner(PlaceholderJSMixin, OptionalAppsMixin,
                         DiscoverRunner):
