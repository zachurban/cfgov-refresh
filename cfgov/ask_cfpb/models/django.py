...
class Answer(models.Model):
    ...
    # def sync_related_questions(self, page):
    #     related = list(self.related_questions.all())
    #     page.related_questions.add(*related)
    ...
    def create_or_update_page(self, language=None):
        ...
        _page.content = StreamValue(
            ...
            is_lazy=True)
        # self.sync_related_questions(_page)
    ...
    def save(self, skip_page_update=False, *args, **kwargs):
        if not self.id:
            ...
            if skip_page_update is False:
                ...
                if self.english_page and self.english_page.redirect_to is None:
                    self.create_or_update_page(language='en')
                elif not self.english_page:
                    self.create_or_update_page(language='en')
                # if self.update_spanish_page:
                if self.spanish_page and self.spanish_page.redirect_to is None:
                    self.create_or_update_page(language='es')
                elif not self.spanish_page:
                    self.create_or_update_page(language='en')
